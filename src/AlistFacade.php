 ...
 * @method static array upload(string $filePath, string $alistFolder, string $type = HttpEnum::TYPE_MULTIPART) 上传文件/流式上传文件
 * @method static array delete(string $alistFolder, array $removeFiles) 删除文件或文件夹
 * @method static array mkdir(string $path) 新建文件夹
 * @method static array rename(string $alistPath, string $name) 重命名
 * @method static array batchRename(string $alistFolder, array $renameObjects) 批量重命名 "rename_objects":[{"src_name":"a","new_name":"b",}]
 * @method static array move(string $oldFolder, string $newFolder, array $moveFiles) 移动文件
 * @method static array recursiveMove(string $oldFolder, string $newFolder) 聚合移动
 * @method static array removeEmptyDirectory(string $alistFolder) 删除空文件夹
